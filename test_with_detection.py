...
model.load_state_dict(torch.load('./work_space/exp/model_best_9.pt')) ##  under fit  epoch 700
...
fast_rcnn = torch.load('./work_space/exp/model_200.pth') # 207
fast_rcnn.to('cpu')
fast_rcnn.eval()
...
with torch.no_grad():
    for idx in range(1,3):
        print('-----------: ', idx)
        ## 读取图像
        img_o = cv2.imread('./example/{}.jpg'.format(idx))
        depth = cv2.imread('./example/{}.png'.format(idx), cv2.IMREAD_ANYDEPTH)
        start = time.time()
        ...
        with torch.no_grad():
            prediction = fast_rcnn([rgb.to('cpu')])
        ...
        for b_idx in range(boxes.shape[0]):
            if scores[b_idx] >= 0.7:
                x1, y1, x2, y2 = int(boxes[b_idx][0]), int(boxes[b_idx][1]), int(boxes[b_idx][2]), int(boxes[b_idx][3])
        ...
        all_score = []
        all_dis = []
        for b_idx in range(len(all_point)):
            x, y = all_point[b_idx]
            score = 0
            dis_m = 0
        ...
        index = index_d
        # cv2.circle(img_o, (all_point[index][0], all_point[index][1]), 3, (0, 255, 255), 2)
        ...
        # pre_pose,_ = model.read_network_output()
        ...
        cv2.circle(grasp_kps_map, (gk_point[0][1], gk_point[0][0]), 3, (255, 0, 0), 2)  # 抓取关键点1
        cv2.circle(grasp_kps_map, (gk_point[1][1], gk_point[1][0]), 3, (0, 255, 0), 2)  # 抓取关键点2
        cv2.circle(grasp_kps_map, (g_point[0][1], g_point[0][0]), 3, (0, 0, 255), 2)  # 抓取关键点2
