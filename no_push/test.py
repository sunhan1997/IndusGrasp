...
model.load_state_dict(torch.load('/home/sunh/github/IndusGrasp/work_space/exp/model_best_139.pt')) ## minxtyle
print('cuda is ok or not:  ', torch.cuda.is_available())
model.cuda()
model.eval()
...
device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
#  载入fast rcnn训练结果 这里使用的是torchvision的
fast_rcnn = torch.load('./maskrcnn/model.pth') # 207
fast_rcnn.to(device)
fast_rcnn.eval()


with torch.no_grad():
    for idx in range( 0, 1400): #
        print('-----------: ', idx)
        # img,fusion, gt_pose, path = dataset.torch_batch([idx])
        ## 读取图像
        img_o = cv2.imread('/home/sunh/6D_ws/my_dataset/ObjectDatasetTools-master/LINEMOD/gr1/JPEGImages/{}.jpg'.format(idx))
        ...
        img_fusion = dataset.gen_multi_feature_fusion_map(rgb)
        ...
        # cv2.imwrite('/home/sunh/Downloads/output_5.png',img_o)
